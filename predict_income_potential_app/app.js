	...
	request(options, function(err, res, body) {
		...
		request(opts, function(err, res, body) {
			...
			for (i = 0; i < body.resources.length; i++) {
				if (body.resources[i].entity.published_model.name == 'predict_income_potential_Model') {
					...
					console.log('predict_income_potential_Model');
					break;
					}
			}
			if (!scoringHref) {
				console.log('Error: Did not find predict_income_potential Deployment Model');
			}
		});
	});
}
